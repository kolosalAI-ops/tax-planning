...
export default function PipelinePage() {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-[#0D0E0F]">Tax Optimization Pipeline</h1>
        <p className="text-sm text-[#6A6F73] mt-1">B2B startup structures, product principal models, and revenue sensitivity analysis</p>
      </div>
            ...
            <tbody>
              {topStructures.map((s, i) => (
                <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F1F3F4]'}>
                  <td className="px-3 py-2 font-mono font-bold text-[#0052C4]">{i + 1}</td>
          ...
          {singleRanking.map((r, i) => {
            ...
            return (
              <div key={r.country} className="flex items-center gap-3">
                <span className="w-5 text-[11px] font-mono font-bold text-[#0052C4]">{i + 1}</span>
                <div className="w-28 flex items-center gap-1.5">
                  <span className="text-sm">{COUNTRY_FLAGS[r.country]}</span>
                  <span className="text-[11px] font-semibold text-[#0D0E0F]">{COUNTRY_LABELS[r.country]}</span>
                </div>
                <div className="flex-1 h-6 bg-[#F1F3F4] rounded overflow-hidden relative">
                  <div
                    className="h-full rounded flex items-center pl-2"
                    style={{
                      width: `${barWidth}%`,
                      backgroundColor: r.effectiveRate <= 10 ? '#0052C4' : r.effectiveRate <= 20 ? '#0066F5' : r.effectiveRate <= 30 ? '#CC8727' : '#CC2727',
                    }}
                  >
          ...
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {revSensitivity.map((r, i) => (
                <tr key={r.label} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F1F3F4]'}>
                  <td className="px-3 py-2 font-semibold text-[#0D0E0F] whitespace-nowrap">{r.label}</td>
                  {(['US', 'CA', 'SG', 'ID', 'AU', 'MX', 'BR'] as const).map((code) => {
                    const val = r[code];
                    const isBest = r.best === code;
                    return (
                      <td key={code} className={`px-3 py-2 text-right font-mono ${isBest ? 'font-bold text-[#0052C4]' : 'text-[#0D0E0F]'}`}>
                        {val.toFixed(1)}%
                      </td>
                    );
                  })}
                  <td className="px-3 py-2 text-[#0052C4] font-bold whitespace-nowrap">
      ...
      <div className="bg-white border border-[#E4E7E9] rounded-lg overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {ppRankings.map((r, i) => (
                <tr key={r.principal} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F1F3F4]'}>
                  <td className="px-3 py-2 font-mono font-bold text-[#0052C4]">{i + 1}</td>
                  ...
                  <td className="px-3 py-2 text-[#6A6F73] max-w-xs truncate" title={r.keyAdvantage}>{r.keyAdvantage}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div>
        <h2 className="text-sm font-semibold text-[#6A6F73] uppercase tracking-wider mb-3">Architectures</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {ppArchitectures.map((arch) => (
            <div key={arch.id} className="bg-white border border-[#E4E7E9] rounded-lg overflow-hidden">
              <div className="px-4 py-3 border-b border-[#E4E7E9] bg-[#F1F3F4]">
              ...
              <div className="p-3 space-y-1.5">
                <div className="flex items-center gap-1.5 text-[11px]">
                  <span className="text-[#0052C4] font-bold">Principal:</span>
              ...
              <div className="px-3 pb-3">
                <div className="flex flex-wrap gap-1">
                  {arch.legalBasis.map((lb) => (
                    <span key={lb} className="text-[9px] font-mono bg-[#F1F3F4] text-[#6A6F73] px-1.5 py-0.5 rounded">
                      {lb}
                    </span>
                  ))}
                </div>
